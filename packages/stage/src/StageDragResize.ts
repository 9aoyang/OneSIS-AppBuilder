...
export default class StageDragResize extends EventEmitter {
  ...
  private update(isResize = false, parentEl: HTMLElement | null = null): void {
    ...
    const offset =
      this.mode === Mode.SORTABLE
        ? { left: 0, top: 0 }
        : { left: this.target.offsetLeft, top: this.target.offsetTop };
    ...
    if (parentEl && this.mode === Mode.ABSOLUTE && this.dragEl) {
      const [translateX, translateY] = this.moveableHelper?.getFrame(this.dragEl).properties
      ...
      top =
        calcValueByFontsize(doc, this.dragEl.offsetTop) +
  ...
  private getOptions(options: MoveableOptions = {}): MoveableOptions {
    ...
    const elementGuidelines: any =
      moveableOptions.elementGuidelines || this.target.parentElement?.children || [];
