...
export default class StageMask extends Rule {
  ...
  public observe(page: HTMLElement): void {
    ...
    this.page = page;
    this.pageScrollParent =
      getScrollParent(page) || this.core.renderer.contentWindow?.document.documentElement || null;
  ...
  private mouseDownHandler = (event: MouseEvent): void => {
    ...
    if (
      ...
      (event.target as HTMLDivElement).className.indexOf('moveable-area') !== -1
    ) {
      return;
    }
