...
export default class StageCore extends EventEmitter {
  ...
  constructor(config: StageCoreConfig) {
    ...
    this.containerHighlightClassName =
      config.containerHighlightClassName || CONTAINER_HIGHLIGHT_CLASS;
    ...
    this.mask
      ...
      .on('beforeMultiSelect', async (event: MouseEvent) => {
        ...
        // 判断元素是否已在多选列表
        const existIndex = this.selectedDomList.findIndex(
  ...
  public async multiSelect(idOrElList: HTMLElement[] | Id[]): Promise<void> {
    ...
    const elList = await Promise.all(
      idOrElList.map(async (idOrEl) => await this.getTargetElement(idOrEl))
    );
