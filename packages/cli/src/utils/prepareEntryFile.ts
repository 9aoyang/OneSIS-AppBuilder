...
export const prepareEntryFile = (app: App) => {
  const {
    ...
    valueMap = {},
    eventMap = {},
  } = app.moduleMainFilePath;
  const { componentFileAffix } = app.options;

  app.writeTemp(
    'comp-entry.ts',
    generateContent(EntryType.COMPONENT, componentMap, componentFileAffix)
  );
...
const prettyCode = (code: string) =>
  recast.prettyPrint(
    recast.parse(code.replace(/\\/g, '/'), { parser: require('recast/parsers/typescript') }),
    {
      tabWidth: 2,
      trailingComma: true,
      quote: 'single',
