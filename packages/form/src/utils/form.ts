...
const asyncLoadConfig = (
  value: FormValue,
  initValue: FormValue,
  { asyncLoad, name, type }: HtmlField
) => {
  // 富文本配置了异步加载
...
const isMultipleValue = (type?: string | TypeFunction) =>
  typeof type === 'string' &&
  ['checkbox-group', 'checkboxGroup', 'table', 'cascader', 'group-list', 'groupList'].includes(
    type
  );
...
const initItemsValue = (
  ...
  if (Array.isArray(initValue[name])) {
    value[name] = initValue[name].map((v: any, index: number) =>
      init(mForm, items, v, value[name]?.[index])
    );
  } else {
  ...
  if (name && !items && typeof initValue[name] !== 'undefined') {
    if (typeof value[name] === 'undefined') {
      if (type === 'number') {
        ...
        value[name] =
          typeof initValue[name] === 'object' ? cloneDeep(initValue[name]) : initValue[name];
      }
    }
...
const getDefaultValue = function (
  mForm: FormState | undefined,
  { defaultValue, type, filter, multiple }: DefaultItem
) {
